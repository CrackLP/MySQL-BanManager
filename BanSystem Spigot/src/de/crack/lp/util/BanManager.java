...
public class BanManager {
	...
	public static void ban(String uuid, String playername, String reason, long seconds) {
		...
		if (getBanCount(uuid) == 0)
			MySQL.update("INSERT INTO bancounts (Spielername, UUID, Count) VALUES ('" + playername + "', '" + uuid
					+ "', '1')");
		else
			MySQL.update("UPDATE bancounts SET Count = '" + (getBanCount(uuid) + 1) + "' WHERE UUID='" + uuid + "'");
	...
	public static String getRemainingTime(String uuid) {
		...
		return new SimpleDateFormat("dd.MM.yyyy").format(c.getTime()) + " um "
				+ new SimpleDateFormat("HH:mm:ss").format(c.getTime());
	}

	public static int getBanCount(String uuid) {
		ResultSet rs = MySQL.getResult("SELECT * FROM BanCounts WHERE UUID='" + uuid + "'");
		try {
			if (rs.next()) {
